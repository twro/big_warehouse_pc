...
!function (window) {
    ...
    var util = ydui.util = {
         ...
         * 椤甸潰婊氬姩鏂规硶銆愮Щ鍔ㄧ銆�
         * @type {{lock, unlock}}
         * lock锛氱姝㈤〉闈㈡粴鍔�, unlock锛氶噴鏀鹃〉闈㈡粴鍔�
         */
        pageScroll: function () {
            var fn = function (e) {
                e.preventDefault();
...
!function (window, ydui) {
    ...
    /**
     * 纭鎻愮ず妗�
     * @param title 鏍囬String 銆愬彲閫夈€�
     * @param mes   鍐呭String 銆愬繀濉€�
     * @param opts  鎸夐挳浠珹rray 鎴� 鈥滅‘瀹氭寜閽€濆洖璋冨嚱鏁癋unction 銆愬繀濉€�
     * @constructor
     */
    ...
    /**
     * 寮瑰嚭璀︾ず妗�
     * @param mes       鎻愮ず鏂囧瓧String 銆愬繀濉€�
     * @param callback  鍥炶皟鍑芥暟Function 銆愬彲閫夈€�
     */
    ...
    dialog.toast = function () {
        ...
        /**
         * @param mes       鎻愮ず鏂囧瓧String 銆愬繀濉€�
         * @param type      绫诲瀷String success or error 銆愬繀濉€�
         * @param timeout   澶氫箙鍚庢秷澶盢umber 姣 銆愰粯璁わ細2000ms銆戙€愬彲閫夈€�
         * @param callback  鍥炶皟鍑芥暟Function 銆愬彲閫夈€�
         */
...
!function (window) {
    ...
    InfiniteScroll.DEFAULTS = {
        binder: window, // 缁戝畾娴忚鍣ㄦ粴鍔ㄤ簨浠禗OM
        initLoad: true, // 鏄惁鍒濆鍖栧姞杞界涓€灞忔暟鎹�
        pageSize: 0, // 姣忛〉璇锋眰鐨勬暟鎹噺
        loadingHtml: '鍔犺浇涓�...', // 鍔犺浇涓彁绀猴紝鏀寔HTML
        doneTxt: '娌℃湁鏇村鏁版嵁浜�', // 鍔犺浇瀹屾瘯鎻愮ず
        backposition: false, // 鏄惁浠庤鎯呴〉杩斿洖鍒楄〃椤甸噸鏂板畾浣嶄箣鍓嶄綅缃�
        jumpLink: '', // 璺宠浆璇︽儏椤甸摼鎺ュ厓绱�
        loadListFn: null, // 鍔犺浇鏁版嵁鏂规硶
        loadStorageListFn: null // 鍔犺浇SesstionStorage鏁版嵁鏂规硶
    };
    ...
    InfiniteScroll.prototype.bindScrollEvent = function () {
        ...
        $binder.on('scroll.ydui.infinitescroll', function () {
            ...
            // 褰撴祻瑙堝櫒婊氬姩鍒板簳閮ㄦ椂锛屾鏃� _this.$tag.offset().top 绛変簬 contentTop + contentHeight
    ...
    InfiniteScroll.prototype.loadList = function () {
        ...
        if (typeof options.loadListFn == 'function') {
            ...
            options.loadListFn().done(function (listArr, page) {
                ...
                // 褰撹姹傜殑鏁版嵁灏忎簬pageSize[姣忛〉璇锋眰鏁版嵁鏁癩锛屽垯璁や负鏁版嵁鍔犺浇瀹屾瘯锛屾彁绀虹浉搴斾俊鎭�
    ...
    InfiniteScroll.prototype.loadListFromStorage = function () {
        ...
        // 灏嗘暟鎹紶鍑哄閮ㄦ柟娉曪紝鐩磋嚦鍏堕€氱煡宸叉彃鍏ラ〉闈㈠悗婊氬姩鑷崇浉搴斾綅缃�
...
!function () {
    ...
    SendCode.DEFAULTS = {
        run: false, // 鏄惁鑷姩鍊掕鏃�
        secs: 60, // 鍊掕鏃舵椂闀匡紙绉掞級
        disClass: '', // 绂佺敤鎸夐挳鏍峰紡
        runStr: '{%s}绉掑悗閲嶆柊鑾峰彇', // 鍊掕鏃舵樉绀烘枃鏈�
        resetStr: '閲嶆柊鑾峰彇楠岃瘉鐮�' // 鍊掕鏃剁粨鏉熷悗鎸夐挳鏄剧ず鏂囨湰
    };
...
!function (window) {
    ...
    Slider.DEFAULTS = {
        speed: 300, // 绉诲姩閫熷害
        autoplay: 3000, // 寰幆鏃堕棿
        lazyLoad: false, // 鏄惁寤惰繜鍔犺浇鍥剧墖 data-src=""
    ...
    /**
     * 宸﹀彸婊戝姩Slider
     * @param speed 绉诲姩閫熷害 0锛氬綋鍓嶆槸鍋峰伔鎽告懜鐨勭Щ鍔ㄥ暒锛岀敓鎬曠粰浣犵湅瑙�
     * @param x 妯悜绉诲姩瀹藉害
     */
    ...
    Slider.prototype.touches = {
        moveTag: 0, // 绉诲姩鐘舵€�(start,move,end)鏍囪
        startClientX: 0, // 璧峰鎷栧姩鍧愭爣
        moveOffset: 0, // 绉诲姩鍋忕Щ閲忥紙宸﹀彸鎷栧姩瀹藉害锛�
        touchStartTime: 0, // 寮€濮嬭Е鎽哥殑鏃堕棿鐐�
        isTouchEvent: false, // 鏄惁瑙︽懜浜嬩欢
    ...
    Slider.prototype.onTouchEnd = function () {
        ...
        if (touches.moveTag == 2) {
            ...
            // 璁＄畻寮€濮嬭Е鎽稿埌缁撴潫瑙︽懜鏃堕棿锛岀敤浠ヨ绠楁槸鍚﹂渶瑕佹粦鑷充笅涓€椤�
            var timeDiff = Date.now() - touches.touchStartTime;

            // 鎷栧姩鏃堕棿瓒呰繃300姣鎴栬€呮湭鎷栧姩瓒呰繃鍐呭涓€鍗�
...
!function (window) {
    ...
    Tab.prototype.open = function (index) {
        ...
        // 濡傛灉鍒囨崲鍔ㄧ敾杩涜鏃舵垨鑰呭綋鍓嶄簩娆＄偣鍑� 绂佹閲嶅鎿嶄綔
    ...
    /**
     * 娣诲姞閫変腑鏍峰紡
     * @param $element 褰撳墠闇€瑕佹坊鍔犻€変腑鏍峰紡鐨勫璞�
     * @param $container 褰撳墠瀵硅薄鐨勫悓绾ф墍鏈夊璞�
     * @param callback 鍥炶皟
     */
...
!function (window) {
    ...
    /**
     * 鏃ユ湡鏍煎紡鍖�
     * @param format 鏃ユ湡鏍煎紡 {%d澶﹠{%h鏃秨{%m鍒唥{%s绉拀{%f姣}
     * @param time 鍗曚綅 姣
     * @returns {string}
     */
    ...
    /**
     * js鍊掕鏃�
     * @param format 鏃堕棿鏍煎紡 {%d}澶﹞%h}鏃秢%m}鍒唟%s}绉抺%f}姣
     * @param time 缁撴潫鏃堕棿鏃堕棿鎴� 姣
     * @param speed 閫熷害
     * @param callback ret 鍊掕鏃剁粨鏉熷洖璋冨嚱鏁� ret 鏃堕棿瀛楃 锛況et == '' 鍒欏€掕鏃剁粨鏉�
     * DEMO: YDUI.util.countdown('{%d澶﹠{%h鏃秨{%m鍒唥{%s绉拀{%f姣}', Date.parse(new Date()) + 60000, 1000, function(ret){ console.log(ret); });
     */
     ...
     * js 鍔犲噺涔橀櫎
     * @param arg1 鏁板€�1
     * @param op 鎿嶄綔绗tring 銆�+ - * /銆�
     * @param arg2 鏁板€�2
     * @returns {Object} arg1 涓� arg2杩愮畻鐨勭簿纭粨鏋�
     */
    ...
    util.cookie = function () {
        return {
             ...
             * @param {String}  val 鍊�
             * @param {Number} expires 鍗曚綅锛堢锛�
             * @param {String}  domain 鍩�
             * @param {String}  path 鎵€鍦ㄧ殑鐩綍
             * @param {Boolean} secure 璺ㄥ崗璁紶閫�
             */
            set: function (name, val, expires, domain, path, secure) {
                var text = String(encodeURIComponent(val)),
                    date = expires;

                // 浠庡綋鍓嶆椂闂村紑濮嬶紝澶氬皯灏忔椂鍚庤繃鏈�
