...
export default {
  ...
  methods: {
    ...
    rotating() {
      ...
      if (this.index > len) {
        this.index = this.prize_list.findIndex(item => {
          return item.isPrize == 0;
        });
      }
      if (type == 0) {
        // 转动盘子
        // (result_angle[result_index] ? result_angle[result_index] : 0)

        var rotate_angle =
          this.start_rotating_degree +
          rand_circle * 360 +
          (360 - this.index * (360 / len) - 360 / len / 2) -
    ...
    game_over() {
      ...
      this.hasPrize = this.prize_list[this.index]
